...
def evaluate_performance(scores: pd.DataFrame) -> dict:
    ...
    try:
        ...
        logger.debug("MAE metric calculated. MAE = %0.4f", mae)
    ...
    try:
        ...
        logger.debug("MSE metric calculated. MSE = %0.4f", mse)
    ...
    try:
        ...
        logger.debug("RMSE metric calculated. PrecRMSEision = %0.4f", rmse)
    ...
    try:
        ...
        logger.warning("An error occured when calculating R2 metric. The process will  " +
                       "continue without R2. Error: %s", err)
    else:
        eval_metrics["R2"] = r2
        logger.debug("R2 metric calculated. R2 = %0.4f", r2)

    
    logger.info("Evaluation metrics calculated. Evaluated metrics that were successfull: %s",
                 eval_metrics)
